...
contract Lighthouse {
    ...
    modifier onlyOwner  {
        require(owner == msg.sender, "Unauthorised access");
        _;
    }
    ...
    function changeAuth(address newAuth) public onlyOwner {
        auth = newAuth;
    }
    ...
    function changeSearcher(Searcher newSeeker) public onlyOwner {
        seeker = newSeeker;
        require(seeker.identify() == 0xda4b055,"invalid searcher");
    }

    function setMaxAge(uint newMaxAge) public onlyOwner {
        maxAge = newMaxAge;
    }
...
contract LighthouseFactory {
    ...
    address public auth = msg.sender; // ownable model.

    Lighthouse[] public lighthouses;
    
    event newLighthouse(address newLighthouse, uint housePosition);
    ...
    function makeLighthouse() public onlyAuth {
        Lighthouse newLight = new Lighthouse();
        newLight.changeAuth(auth);
        newLight.changeOwner(auth);
        uint pos = lighthouses.push(newLight);
        emit newLighthouse(address(newLight),pos);
    }
    
    function numberOfLighthouses() public view returns (uint) {
